...
1.  **Media Extraction**: (Completed) Logic to extract images (`<img>` tags) is implemented in `arun` and populated in `CrawlResult.media`. The structure handles `src`, `alt`, `desc`, etc.
2.  **Link Extraction**: (Completed) Logic to extract links (`<a>` tags) is implemented in `arun` and populated in `CrawlResult.links`. Currently categorizes all as "internal" - better categorization logic is needed.
